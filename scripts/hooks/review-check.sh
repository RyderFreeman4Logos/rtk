...
# Skip the review requirement on protected branches: a direct push to them is
# blocked outright by branch-protection.sh (wired into BOTH pre-commit and
# pre-push), so this gate never needs to demand a review there. Keep this list
# in sync with PROTECTED in branch-protection.sh (main dev master).
...
# SECURITY: embed only SAFE_BRANCH (restricted to [a-zA-Z0-9._-] by
# _sanitize_branch), never the raw CURRENT_BRANCH. Git permits branch names
# containing <, >, ", and whitespace; a crafted name could otherwise break out
# of this HTML-comment control marker and inject attacker-controlled directives
# into the agent's context. CURRENT_HEAD is a hex SHA, so it is safe as-is.
cat >&2 <<GATE_BLOCKED
